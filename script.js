...
(function() {
    const STORAGE_KEY = 'diario_viagem';

    // ── Nav ──────────────────────────────
    const mainPlaylist = document.querySelector('.main-content:not(.diario-view)');
    const mainDiario   = document.getElementById('diario-view');
    ...
    function saveEntradas(list) {
        localStorage.setItem(STORAGE_KEY, JSON.stringify(list));
    }

    // ── Abrir/fechar form ─────────────────
    const modal         = document.getElementById('diario-modal');
    const btnAbrir      = document.getElementById('abrir-form-diario');
    const btnFechar     = document.getElementById('fechar-form-diario');
    const btnCancelar   = document.getElementById('cancelar-diario');
    const btnSalvar     = document.getElementById('salvar-diario');

    const inputTitulo   = document.getElementById('diario-titulo');
    const inputMensagem = document.getElementById('diario-mensagem');
    const inputLugar    = document.getElementById('diario-lugar');
    const inputFoto     = document.getElementById('diario-foto-input');
    const uploadArea    = document.getElementById('diario-upload-area');
    const placeholder   = document.getElementById('diario-upload-placeholder');
    const preview       = document.getElementById('diario-preview');
    const btnRemoverFoto= document.getElementById('diario-remover-foto');
    ...
    function fecharForm() { modal.style.display = 'none'; }

    btnAbrir.addEventListener('click', abrirForm);
    btnFechar.addEventListener('click', fecharForm);
    btnCancelar.addEventListener('click', fecharForm);
    modal.addEventListener('click', e => { if (e.target === modal) fecharForm(); });
    ...
    btnSalvar.addEventListener('click', () => {
        ...
        if (!titulo && !msg) {
            inputTitulo.focus();
            inputTitulo.style.borderColor = 'var(--rose)';
            setTimeout(() => inputTitulo.style.borderColor = '', 1200);
            return;
        }
        const entrada = {
            id:       Date.now(),
        ...
        // feedback visual
        lancarCoracoes && lancarCoracoes();
    });

    // ── Renderizar lista ──────────────────
    function renderDiario() {
        const lista = getEntradas();
        const container = document.getElementById('diario-lista');
        const vazio     = document.getElementById('diario-vazio');
        const count     = document.getElementById('diario-count');

        // limpar cards (preservar vazio)
        container.querySelectorAll('.diario-card').forEach(c => c.remove());

        if (!lista.length) {
            vazio.style.display = '';
            count.textContent   = 'nenhuma entrada ainda';

            // Atualizar hero img
            const heroImg = document.getElementById('diario-hero-img');
            if (heroImg) heroImg.src = 'covers/romantic.jpg';
            return;
        }

        vazio.style.display = 'none';
        count.textContent   = `${lista.length} entrada${lista.length !== 1 ? 's' : ''}`;

        // Hero img = foto da entrada mais recente com foto
        const comFoto = lista.find(e => e.foto);
        const heroImg = document.getElementById('diario-hero-img');
        if (comFoto && heroImg) heroImg.src = comFoto.foto;

        lista.forEach(entrada => {
            const card = document.createElement('div');
            card.className = 'diario-card';
            card.innerHTML = entrada.foto
                ? `<img class="diario-card-foto" src="${entrada.foto}" alt="${entrada.titulo}" loading="lazy">`
                : `<div class="diario-card-semfoto">📸</div>`;
            card.innerHTML += `
                <div class="diario-card-body">
                    ${entrada.lugar ? `<p class="diario-card-lugar">📍 ${entrada.lugar}</p>` : ''}
                    <h3 class="diario-card-titulo">${entrada.titulo}</h3>
                    ${entrada.mensagem ? `<p class="diario-card-preview">${entrada.mensagem}</p>` : ''}
                </div>
                <div class="diario-card-footer">
                    <span class="diario-card-data">🗓️ ${entrada.data}</span>
                    <span style="color:var(--green);font-size:13px">ver →</span>
                </div>
            `;
            card.addEventListener('click', () => verEntrada(entrada));
            container.appendChild(card);
        });
    }

    // ── Ver entrada ───────────────────────
    const verModal      = document.getElementById('diario-ver-modal');
    const verFotoWrap   = document.getElementById('diario-ver-foto-wrap');
    const verFoto       = document.getElementById('diario-ver-foto');
    const verLugar      = document.getElementById('diario-ver-lugar');
    const verTitulo     = document.getElementById('diario-ver-titulo');
    const verData       = document.getElementById('diario-ver-data');
    const verMensagem   = document.getElementById('diario-ver-mensagem');
    const btnFecharVer  = document.getElementById('fechar-ver-diario');
    const btnApagar     = document.getElementById('diario-apagar-btn');
    let entradaAberта   = null;

    function verEntrada(entrada) {
        entradaAberта = entrada;
        if (entrada.foto) {
            verFoto.src = entrada.foto;
            verFotoWrap.style.display = '';
        } else {
            verFotoWrap.style.display = 'none';
        }
        if (entrada.lugar) {
            verLugar.textContent = '📍 ' + entrada.lugar;
            verLugar.style.display = '';
        } else {
            verLugar.style.display = 'none';
        }
        verTitulo.textContent   = entrada.titulo;
        verData.textContent     = entrada.data;
        verMensagem.textContent = entrada.mensagem || '';
        verModal.style.display  = 'flex';
    }

    function fecharVer() { verModal.style.display = 'none'; entradaAberта = null; }
    btnFecharVer.addEventListener('click', fecharVer);
    verModal.addEventListener('click', e => { if (e.target === verModal) fecharVer(); });

    btnApagar.addEventListener('click', () => {
        if (!entradaAberта) return;
        if (!confirm('Apagar essa entrada? Não tem como desfazer.')) return;
        const list = getEntradas().filter(e => e.id !== entradaAberта.id);
        saveEntradas(list);
        fecharVer();
        renderDiario();
    });

    // Renderizar ao carregar se já estiver na view
    if (mainDiario && mainDiario.style.display !== 'none') renderDiario();
})();
